...
Given(/^I have a sample compound metric configuration within the given mezuro configuration$/) do
  @compound_metric_configuration = FactoryGirl.create(:compound_metric_configuration, {configuration_id: @mezuro_configuration.id, reading_group_id: @reading_group.id})
end

Given(/^I have another compound metric configuration with code "(.*?)" within the given mezuro configuration$/) do |code|
  @another_compound_metric_configuration = FactoryGirl.create(:compound_metric_configuration, {configuration_id: @mezuro_configuration.id, code: code, reading_group_id: @reading_group.id})
end
...
When(/^I click the show link of "(.*?)"$/) do |name|
  page.find('tr', :text => I18n.t(name)).click_link(I18n.t('show'))
end
